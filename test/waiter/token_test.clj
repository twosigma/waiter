    ...
    (let [can-run-as? (fn [auth-user run-as-user] (= auth-user run-as-user))
      ...
      (testing "test:post-new-service-description:schema-fail"
        (let [kv-store (kv/->LocalKeyValueStore (atom {}))
              ...
              {:keys [status body]}
              (handle-token-request
                kv-store waiter-hostname can-run-as? make-peer-requests-fn #(sd/validate-schema % nil)
...
(deftest test-handle-refresh-token-request
  (let [kv-store (kv/->LocalKeyValueStore (atom {}))
        token "test-token"]
    (testing "test:empty-token"
      (let [{:keys [body status]}
        ...
        (is (= 400 status))
        (is (every? #(str/includes? (str body) (str %)) ["Token is missing!"]))))

    (testing "test:non-empty-token"
      (let [{:keys [body status]}
            (handle-refresh-token-request
              kv-store
              token
              ...
              {:authorization/user "test-user"})]
        (is (= 200 status))
        (is (every? #(str/includes? (str body) (str %)) ["\"success\":true", token]))))))
