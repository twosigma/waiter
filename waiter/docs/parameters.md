...
|`x-waiter-service-mapping`|"legacy"|"legacy" or "exclusive"|Determines whether tokens having the same configuration will map to the same service (legacy mode) or to different services (exclusive mode).|Don't change it unless you want services unique to your token.|
