...
(defn- run-handle-token-request
  [kv-store custom-components token-root waiter-hostnames entitlement-manager make-peer-requests-fn validate-service-description-fn
   attach-service-defaults-fn test-post-validator-fn request]
  (let [cluster-calculator (new-configured-cluster-calculator {:default-cluster (str token-root "-cluster")
                                                               :host->cluster {}})]
    (handle-token-request clock custom-components synchronize-fn kv-store cluster-calculator token-root history-length limit-per-owner
                          waiter-hostnames entitlement-manager make-peer-requests-fn validate-service-description-fn
                          attach-service-defaults-fn (au/latest-chan) test-post-validator-fn request)))
  ...
  (with-redefs [sd/service-description->service-id (fn [prefix sd] (str prefix (hash (select-keys sd sd/service-parameter-keys))))]
    (let [kv-store (kv/->LocalKeyValueStore (atom {}))
      ...
      (testing "put:unsupported-request-method"
        (let [{:keys [body status]}
              (run-handle-token-request
                kv-store nil token-root waiter-hostnames entitlement-manager make-peer-requests-fn nil attach-service-defaults-fn (constantly nil)
                {:headers {}
                 :request-method :put})]
      ...
      (testing "post:new-service-description"
        (let [{:keys [body headers status]}
              (run-handle-token-request
                kv-store nil token-root waiter-hostnames entitlement-manager make-peer-requests-fn (constantly true) attach-service-defaults-fn (constantly nil)
                {:authorization/user auth-user
      ...
      (testing "post:new-service-description-different-owner"
        (let [token (str token "-tu")
              {:keys [body status]}
              (run-handle-token-request
                kv-store nil token-root waiter-hostnames (public-entitlement-manager) make-peer-requests-fn (constantly true) attach-service-defaults-fn (constantly nil)
                {:authorization/user auth-user
      ...
      (testing "post:post-validator-fn-throws-error"
        (let [custom-comp {:fee :fie :foe :fum}
              kv-store (kv/->LocalKeyValueStore (atom {}))
              post-validator-fn (fn post-validator [custom-comp-arg _ _]
                                  (is (= custom-comp-arg custom-comp))
                                  (throw (ex-info "post validator failed request" {:status http-400-bad-request})))
              {:keys [body status]}
              (run-handle-token-request
                kv-store custom-comp token-root waiter-hostnames entitlement-manager make-peer-requests-fn (constantly true) attach-service-defaults-fn post-validator-fn
                {:authorization/user auth-user
                 :body (StringBufferInputStream. (utils/clj->json service-description-1))
                 :headers {}
                 :request-method :post})]
          (is (= http-400-bad-request status))
          (is (str/includes? body "post validator failed request")))))))
    ...
    (let [entitlement-manager (authz/->SimpleEntitlementManager nil)
      ...
      (testing "post:new-service-description:schema-fail"
        (let [kv-store (kv/->LocalKeyValueStore (atom {}))
              ...
              {:keys [body status]}
              (run-handle-token-request
                kv-store nil token-root waiter-hostnames entitlement-manager make-peer-requests-fn validate-service-description-fn attach-service-defaults-fn (constantly nil)
                {:authorization/user auth-user
                 :body (StringBufferInputStream. (utils/clj->json service-description))
                 :headers {"x-waiter-token" token}
                 :request-method :post})]
