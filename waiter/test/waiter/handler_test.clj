...
(deftest test-drain-handler
  (let [now (t/now)
    ...
    (testing ":post updates :drain-until using the 'drain-timeout-secs'"
      (let [request {:query-string "drain-timeout-secs=10&crash=false"
                     :request-method :post}
            drain-atom (atom {})
        ...
        (is (= @drain-atom
               {:crash-process? false
                :drain-until (t/plus now (t/seconds 10))}))))

    (testing ":post overrides crash to false if query param 'crash' is not 'true'"
      (let [request {:query-string "drain-timeout-secs=10&crash=foobar"
                     :request-method :post}
    ...
    (testing ":post will attempt to crash if 'crash' is 'true' after the provided 'drain-timeout-secs'"
      (let [request {:query-string "drain-timeout-secs=1&crash=true"
                     :request-method :post}
        ...
        (is (= @drain-atom
               {:crash-process? true
                :drain-until (t/plus now (t/seconds 1))}))
    ...
    (testing ":post will attempt to stop process from crashing if set 'crash' false before previous timeout reached."
      (let [request {:query-string "drain-timeout-secs=1&crash=true"
        ...
        (let [cancel-req {:query-string "drain-timeout-secs=1&crash=false"
                          :request-method :post}
  ...
  (let [test-user "test-user"
    ...
    (testing "signal-handler:success-true"
      (let [scheduler (reify scheduler/ServiceScheduler
                        (signal-instance [_ service-id instance-id _ _]
                          ...
                          {:success true
                           :message (str "kill successfully sent to " test-instance-id)
                           :status 200}))
            request {:request-method :post
                     :authorization/user test-user
                     :route-params {:instance-id test-instance-id :service-id test-service-id}
    ...
    (testing "signal-handler:User not Auth"
      (let [scheduler (reify scheduler/ServiceScheduler
                        (signal-instance [_ service-id instance-id _ _]
                          (is (= test-service-id service-id))
                          (is (= test-instance-id instance-id))
                          {:message "unexpected call" :success false}))
            request {:request-method :post
    ...
    (testing "signal-handler:success-false"
      (let [scheduler (reify scheduler/ServiceScheduler
        ...
        (is (= "application/json" (get headers "content-type")))
        (let [body-json (json/read-str (str body))]
          (is (= {"message" "no-instance-killed"} (get body-json "signal-response"))))))
