...
(deftest test-mesos-api
  (let [http-client (Object.)
        ...
        mesos-agent-port 9876
        mesos-agent-directory "/some/directory"
        mesos-api (api-factory http-client {} mesos-agent-port mesos-agent-directory)
    ...
    (testing "build-sandbox-path"
      (let [agent-id "salve-is"
            framework-id "framework-id"
            instance-id "instance-id"]
        (is (= (str mesos-agent-directory "/" agent-id "/frameworks/" framework-id "/executors/" instance-id "/runs/latest")
               (build-sandbox-path mesos-api agent-id framework-id instance-id)))
        (is (nil? (build-sandbox-path mesos-api nil framework-id instance-id)))
        (is (nil? (build-sandbox-path mesos-api agent-id nil instance-id)))
        (is (nil? (build-sandbox-path mesos-api agent-id framework-id nil)))))

    (testing "list-directory-content"
      (let [host "www.host.com"]
        (with-redefs [hu/http-request
                      (assert-endpoint-request-method :get (str "http://" host ":" mesos-agent-port "/files/browse"))]
    ...
    (testing "build-directory-download-link"
      (let [host "www.host.com"
        ...
        (is (= (str "http://" host ":" mesos-agent-port "/files/download?path=" directory)
    ...
    (testing "get-agent-state"
      (let [host "www.host.com"]
        (with-redefs [hu/http-request
                      (assert-endpoint-request-method :get (str "http://" host ":" mesos-agent-port "/state.json"))]
