...
(deftest test-compute-back-pressure-buffer-size
  (let [min-buffer-size 1024
        max-buffer-size 4096
        instance-reservation-back-pressure {:max-buffer-size max-buffer-size :min-buffer-size min-buffer-size}]
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 1 "max-instances" 10})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 30 "max-instances" 10})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 60 "max-instances" 10})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 120 "max-instances" 10})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 200 "max-instances" 10})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 30 "max-instances" 100})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 60 "max-instances" 100})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 120 "max-instances" 100})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 200 "max-instances" 100})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 30 "max-instances" 200})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 60 "max-instances" 200})))
    (is (= min-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 120 "max-instances" 200})))
    (is (= 1200 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 200 "max-instances" 200})))
    (is (= 2000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 30 "max-instances" 500})))
    (is (= 2500 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 60 "max-instances" 500})))
    (is (= 2500 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 120 "max-instances" 500})))
    (is (= 3000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 200 "max-instances" 500})))
    (is (= 2000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 2 "max-instances" 1000})))
    (is (= 2000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 4 "max-instances" 1000})))
    (is (= 3000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 8 "max-instances" 1000})))
    (is (= 4000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 2 "max-instances" 2000})))
    (is (= 4000 (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 4 "max-instances" 2000})))
    (is (= max-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 8 "max-instances" 2000})))
    (is (= max-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 2 "max-instances" 4000})))
    (is (= max-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 4 "max-instances" 4000})))
    (is (= max-buffer-size (compute-back-pressure-buffer-size instance-reservation-back-pressure {"concurrency-level" 8 "max-instances" 4000})))
