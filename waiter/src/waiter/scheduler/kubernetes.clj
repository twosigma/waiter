...
(defn pod->ServiceInstance
  ...
  (try
    (let [;; waiter-app is the first container we register
          ...
          has-envoy? (contains? (get-in pod [:spec :containers]) 2)
...
(defn default-replicaset-builder
  ...
  (let [work-path (str "/home/" run-as-user)
        ...
        has-envoy? (get env "GRPC_TRANSCODER")
        ...
        port0 (generate-port-with-offset service-id pod-base-port (if has-envoy? 1 0))
