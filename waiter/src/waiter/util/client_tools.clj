...
(defn service-settings
  ...
  [waiter-url service-id & {:keys [cookies headers keywordize-keys query-params]
                            :or {cookies [] headers {} keywordize-keys true query-params {}}}]
  (let [settings-path (str "/apps/" service-id)
        settings-result (make-request waiter-url settings-path :cookies cookies :headers headers :query-params query-params)]
...
(defn delete-token-and-assert
  ...
  (let [request-headers (cond->> headers
    ...
    (when (= response-status http-200-ok)
      (is (= (.valAt response-headers "x-waiter-operation-result") "token-deleted")))))
