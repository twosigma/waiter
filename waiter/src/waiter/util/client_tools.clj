...
(defmacro assert-response-status
  ...
  `(let [response# ~response
         response-cid# (get-in response# [:headers "x-cid"] "unknown")
...
(defmacro assert-waiter-response
  "Asserts the response was generated by Waiter by introspecting the server response header."
  [response]
  `(let [response# ~response]
     (is (str/starts-with? (-> response# :headers (get "server")) "waiter/"))))

(defmacro assert-backend-response
  "Asserts the response was generated by Waiter by introspecting the server response header."
  [response]
  `(let [response# ~response]
     (is (not (str/starts-with? (-> response# :headers (get "server")) "waiter/")))))
...
(defn using-provided-authenticator?
  "Returns true if Waiter is configured to use the provided authenticator"
  [waiter-url]
  (= "provided" (get-authenticator-kind waiter-url)))

(defn supports-provided-authentication?
  "Returns true if Waiter is configured to support provided authentication"
  [waiter-url]
  (or (using-provided-authenticator? waiter-url)
      (and (using-composite-authenticator? waiter-url)
           (get-in (waiter-settings waiter-url) [:authenticator-config :composite :authentication-providers :provided]))))
