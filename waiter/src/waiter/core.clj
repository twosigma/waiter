...
(defn- make-eject-request
  ...
  (log/info "peer communication requesting" dest-router-id "to eject" id "via endpoint" dest-endpoint)
  (try
    (-> (make-inter-router-requests-fn
          dest-endpoint
          :acceptable-router? #(= dest-router-id %)
          :body (utils/clj->json {:instance instance :period-in-ms eject-period-ms :reason reason})
          :method :post)
      (get dest-router-id))
    (catch Exception e
      (log/error e "error in making eject request"
                 {:instance instance :period-in-ms eject-period-ms :reason reason}))))

(defn peers-acknowledged-eject-requests?
  "Note that the ids used in here are internal ids generated by the curator api."
  [{:keys [id] :as instance} short-circuit? router-ids endpoint make-eject-request-fn reason]
  (if (seq router-ids)
    (loop [[dest-router-id & remaining-peer-ids] (seq router-ids)
           eject-successful? true]
      (log/info {:dest-id dest-router-id, :eject-successful? eject-successful?}, :reason reason)
      (let [response (make-eject-request-fn dest-router-id endpoint instance reason)
            response-successful? (= http-200-ok (:status response))
            eject-successful? (and eject-successful? response-successful?)]
        ...
        (if (and remaining-peer-ids (or (not short-circuit?) response-successful?))
          (recur remaining-peer-ids eject-successful?)
          eject-successful?)))
    (do
      (log/warn "no peer routers found to acknowledge eject request!")
      true)))
  ...
  {:async-request-store-atom (pc/fnk [] (atom {}))
   ...
   :scaling-timeout-config (pc/fnk [[:settings
                             ...
                             {:eject-backoff-base-time-ms eject-backoff-base-time-ms
                              ...
                              :max-eject-time-ms max-eject-time-ms})
...
(def routines
  {:admin-user?-fn (pc/fnk [[:state entitlement-manager]]
   ...
   :peers-acknowledged-eject-requests-fn (pc/fnk [[:state discovery router-id]
                                                  make-inter-router-requests-sync-fn]
                                           (fn peers-acknowledged-eject-requests
                                             [instance short-circuit? eject-period-ms reason]
                                             (let [router-ids (discovery/router-ids discovery :exclude-set #{router-id})]
                                               (peers-acknowledged-eject-requests?
                                                 instance short-circuit? router-ids "eject"
                                                 (partial make-eject-request make-inter-router-requests-sync-fn eject-period-ms)
                                                 reason))))
...
(def request-handlers
  {:app-name-handler-fn (pc/fnk [[:routines wrap-service-discovery-fn]
   ...
   :drain-handler-fn (pc/fnk [[:routines admin-user?-fn]
                              [:state clock drain-atom drain-mode?-fn]
                              wrap-secure-request-fn]
                       (wrap-secure-request-fn
                         (fn drain-handler-fn [request]
                           (handler/drain-handler clock drain-atom admin-user?-fn drain-mode?-fn request))))
   :eject-instance-handler-fn (pc/fnk [[:daemons populate-maintainer-chan! router-state-maintainer]
                                       wrap-router-auth-fn]
                                (let [{{:keys [notify-instance-killed-fn]} :maintainer} router-state-maintainer]
                                  (wrap-router-auth-fn
                                    (fn eject-instance-handler-fn [request]
                                      (handler/eject-instance notify-instance-killed-fn populate-maintainer-chan! request)))))
