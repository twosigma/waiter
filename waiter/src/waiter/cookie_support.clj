...
(defn add-cookie
  ...
  [response name value age-in-seconds same-site http-only? secure?]
...
(defn add-encoded-cookie
  ...
  [response password name value age-in-seconds same-site secure?]
  (let [encoded-value (encode-cookie value password)]
    (add-cookie response name encoded-value age-in-seconds same-site true secure?)))
