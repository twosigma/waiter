...
(defn- perform-saml-authentication
  ...
  (let [cookie-jar-file (java.io.File/createTempFile "cookie-jar" ".txt")
        ...
        _ (println "xxxzzz")
        _ (println (str "curl '" saml-redirect-location "' -k -c " cookie-jar-path " -L -v > " curl-output-path))
        xxx (shell/sh "bash" "-c" (str "curl '" saml-redirect-location "' -k -c " cookie-jar-path " -L -v > " curl-output-path))
        _ (println xxx)
        _ (println (slurp curl-output-path))
        curl-verbose-info (:err xxx)
        ...
        _ (println (str "curl '" (str login-form-location login-form-action) "' -k -b " cookie-jar-path " -F 'AuthState=" auth-state "' -F 'username=user2' -F 'password=user2pass' > " curl-output-path))
