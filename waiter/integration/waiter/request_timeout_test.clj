...
(deftest ^:parallel ^:integration-slow ^:dev test-request-queue-timeout-max-instances-limit
  (testing-using-waiter-url
    (let [router-id->router-url (routers waiter-url)
      ...
      (with-service-cleanup
        ...
        (let [timeout-period-ms (/ long-request-period-ms 3)
          ...
          (assert-response-status response 503)
          (is (str/includes? (str body) (str "After " timeout-period-secs " seconds, no instance available to handle request.")))
